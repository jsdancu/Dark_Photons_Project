...
struct vect2{
	...
	int S[80][100]; int B[80][100]; int misID[80][100];
...
//std::cout<<"x: "<<v->x[i] <<"    y: "<< v->y[i]<<std::endl;
							...
							for(Long64_t k=0;k<nentries;k++){
								...
								if(v->id[k] == 22){
									...
									if((v->mother1[i] == v->mother1[j]) && (v->mother1[i] == v->mother1[k]) && (v->mother2[i] == v->mother2[j]) && (v->mother2[i] == v->mother2[k]) && (v->motherid1[i] == 221)){
										...
										if((pt1>mu_ptcut) && (p1>mu_pcut) && (eta1>eta_cut1) && (eta1<eta_cut2) && (pt2>mu_ptcut) &&  (p2>mu_pcut) && (eta2>eta_cut1) && (eta2<eta_cut2) && (inv_mass < m_eta+dm_eta) && (inv_mass> m_eta-dm_eta)){
...
void analyze_event_K(FILE *file_out1, vect *v, vect1 *vmu, vect2 *vgamma, int column, int row, double mu_ptcut, double mu_pcut, double eta_cut1, double eta_cut2, double gamma_ptcut0, double gamma_pcut0, double gamma_ptcut_step, double gamma_pcut_step, double massA, double dmassA, double fiducial_min, double fiducial_max){
...
*/
	...
	TFile  *file_in3 = new TFile("gs_project34.root", "READ");
	...
	fprintf(file_out1, "gamma(pt cut)  gamma(p cut)   S(gamma cuts(pt+p+eta))   B(gamma cuts(pt+p+eta))   misID(gamma cuts(pt+p+eta))   S/sqrt(B)(gamma cuts(pt+p+eta))  \n");
	...
	double index_var, id_var, energy_var, mass_var, px_var, py_var, pz_var, x_var, y_var, z_var, mother1_var, motherid2_var, motherid1_var, mother2_var;
	...
	T2->SetBranchAddress("x",&x_var);
   	T2->SetBranchAddress("y",&y_var);
	T2->SetBranchAddress("z",&z_var);
...
std::cout<<"Total number of entries T2: "<<nentries<<std::endl;
   	for (Long64_t i=0;i<nentries;i++){
		...
		if(i+1==nentries){
			...
			analyze_event_K(file_out1, v, vmu, vgamma, column, row, mu_ptcut, mu_pcut, eta_cut1, eta_cut2, gamma_ptcut0, gamma_pcut0, gamma_ptcut_step, gamma_pcut_step, massA, dmassA, fiducial_min, fiducial_max);
	...
	ptvsp->GetZaxis()->SetRangeUser(1e-10, 1e-9);
	...
	//S_ptvsp->GetZaxis()->SetRangeUser(1e-11, 1e-9);
	...
	//B_ptvsp->GetZaxis()->SetRangeUser(1e-11, 1e-9);
	...
	fprintf(file_out1, "Candidate type    Total entries    After A' mass cut    After eta mass cut   After transverse flight distance cut   Signal significance after mu + A' _fiducial cut: \n");
