...
struct vect2{
	...
	int S1[80][100]; int S2[80][100]; int B[80][100]; int EMB[80][100]; double misID[80][100];
...
*/
	...
	TFile  *file_out = new TFile("gs_project82.root", "recreate");
	...
	FILE  *file_out1 = fopen("results_out82.txt", "w");
...
std::cout<<"Total number of entries T1: "<<nentries<<std::endl;
	...
	for(int i=0;i<column;i++){ 
		...
		for(int j=0;j<row;j++){ 
			...
			B = 1.0;// (double)vgamma->misID[i][j]* RKS;// / redecay;
...
*/
	...
	c1->Modified();
	c1->Update();
	c1->Print("project82_ptvsp.pdf","pdf");

	S_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project82_S_ptvsp.pdf","pdf");

	B_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project82_B_ptvsp.pdf","pdf");	

	KS_number_event->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project82_KS_number_event.pdf","pdf");
	...
	B = 1.0;//B = (double)vmu->misID[2]* RKS;// / redecay;
