...
int main() {
	...
	HepMC::Pythia8ToHepMC ToHepMC;
    	...
    	HepMC::IO_GenEvent ascii_io("gs_project31.dat", std::ios::out);
	...
	pythia.readString("221:onIfMatch = 22 13 -13");

	//Set eta to decay to 2 muons
	//pythia.readString("221:onMode = 1 1 0 13 -13");
	...
	TFile *file = TFile::Open("gs_project31.root","recreate");

	//Create event
	Event *event = &pythia.event;

	//Define a histograms into which we accumulate the invariant mass distribution for 2 muon+gamma combinations
 
    	TH1F *eta_invmass = new TH1F("eta_invmass","Reconstructed eta invariant mass from two muons and gamma", 400, 0.0, 40.0);
	...
	for (int iEvent = 0; iEvent < nEvents; ++iEvent) {
		...
		gamma = false; //sets photons status to false, as in they haven't been saved in the tree
...
std::cout<<"i = "<<i<<"    "<<"j = "<<j<<"    "<<"iEvent = "<<iEvent<<"    "<<"id = "<<id_var<<"    "<< "energy = "<<energy_var<<std::endl;
							...
							for (int k = 0; k < pythia.event.size(); ++k){
								...
								if (pythia.event[k].id()==22){
									...
									//check if the photon has not been filled in the tree (prevent multiple saving of the same photon)
									if(gamma == false){

										++gamma_number;

										//add photon to the tree
										index_var = iEvent;
...
std::cout<<"i = "<<i<<"    "<<"j = "<<j<<"    "<<"k = "<<k<<"    "<<"iEvent = "<<iEvent<<"    "<<"id = "<<id_var<<"    "<< "energy = "<<energy_var<<std::endl;
									...
									//Reconstructing invariant mass of all muon-anti-muon-gamma combinations
							...
							gamma = true;//all photons have been saved in the event
		...
		std::cout<<"final state = "<<final_state<<setw(5)<<"muon number = "<<mu_number<<setw(5)<<"anti muon = "<<antimu_number<<setw(5)<<"photon number = "<<gamma_number<<setw(5)<<"difference = "<<final_state - mu_number - antimu_number-gamma_number<<std::endl;

		total_final_state = total_final_state + final_state;
		total_mu_antimu = total_mu_antimu + mu_number + antimu_number;
		total_gamma = total_gamma + gamma_number;

	}// End event loop.

	std::cout<<"total final state = "<<total_final_state<<setw(5)<<"muon+antimuon number = "<<total_mu_antimu<<setw(5)<<"photon number = "<<total_gamma<<std::endl;
	std::cout<<""<<std::endl;
