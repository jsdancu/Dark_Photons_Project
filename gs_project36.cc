...
int main() {
	...
	TH1D *mu_number_event = new TH1D("mu_number_event","Muon-antimuon number per event without #eta-> #gamma #mu^{+} #mu^{-}", 10000000, 0.0, 10000000.0);
    	mu_number_event -> GetXaxis()-> SetTitle("event index");
	mu_number_event -> GetYaxis()-> SetTitle("number of #mu^{#pm}");

	TH1D *gamma_number_event = new TH1D("gamma_number_event","#gamma number per event without #eta-> #gamma #mu^{+} #mu^{-}", 10000000, 0.0, 10000000.0);
