...
void analyze_event(vect *v, TH1F *eta_invmass, TH1F *muon_invmass_mother, TH1F *eta_invmass_mothers, TH1F *muon_pt_mother, TH1F *muon_ptcut_mother, TH1F *mu_number_event){
...
int main() {
	...
	TFile  *file_in = new TFile("gs_project21.root", "READ");

	// Open the output TFile 
	TFile  *file_out = new TFile("gs_project41.root", "recreate");
	...
	TH1F *muon_invmass_mother = new TH1F("muon_invmass_mother","Reconstructed muon pair invariant mass with same mother", 100, 0.0, 1.0);
    	muon_invmass_mother -> GetXaxis()-> SetTitle("m (GeV)");

	TH1F *eta_invmass_mother = new TH1F("eta_invmass_mother","Reconstructed eta invariant mass from muon pairs with same mother", 100, 0.0, 1.0);
    	eta_invmass_mother -> GetXaxis()-> SetTitle("m (GeV)");

	TH1F *muon_pt_mother = new TH1F("muon_pt_mother","#mu^{-} and #mu^{+} p_{t} distribution with same mother (#eta)", 100, 0.0, 1.0);
...
std::cout<<"Total number of entries: "<<nentries<<std::endl;
	...
	c1->Modified();
	c1->Update();
	c1->Print("project41_etainvmass1.pdf","pdf");

	muon_invmass_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project41_muoninvmass_mother1.pdf","pdf");

	eta_invmass_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project41_etainvmass_mother1.pdf","pdf");
	...
	c1->Modified();
	c1->Update();
	c1->Print("project41_mu_pt_mother1.pdf","pdf");

	muon_ptcut_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project41_mu_ptcut_mother1.pdf","pdf");

	mu_number_event->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project41_mu_number_event.pdf","pdf");
