...
int main() {
	...
	TFile  *file_in = new TFile("gs_project31.root", "READ");

	// Open the output TFile 
	TFile  *file_out = new TFile("gs_project51.root", "recreate");
	...
	TH1F *gamma_number_event = new TH1F("gamma_number_event","#gamma number per event", 10, 0.0, 10.0);
    	gamma_number_event -> GetXaxis()-> SetTitle("event index");
	gamma_number_event -> GetYaxis()-> SetTitle("number of #gamma");

	TH1F *total_number_event = new TH1F("total_number_event","Total number of particles per event", 10, 0.0, 10.0);
...
std::cout<<"Total number of entries: "<<nentries<<std::endl;
	...
	c1->Modified();
	c1->Update();
	c1->Print("project51_etainvmass.pdf","pdf");

	muongamma_invmass_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->SaveAs("project51_muongamma_invmass_mother.pdf","pdf");

	muon_invmass_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project51_muoninvmass_mother.pdf","pdf");

	eta_invmass_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project51_etainvmass_mother.pdf","pdf");

	muon_pt_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project51_mu_pt_mother.pdf","pdf");

	muon_ptcut_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project51_mu_ptcut_mother.pdf","pdf");

	gamma_pt_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project51_gamma_pt_mother.pdf","pdf");

	mu_number_event->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project51_mu_number_event.pdf","pdf");

	gamma_number_event->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project51_gamma_number_event.pdf","pdf");

	total_number_event->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project51_total_number_event.pdf","pdf");
