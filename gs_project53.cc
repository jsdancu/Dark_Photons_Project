...
void analyze_event(vect *v, TH1D *eta_invmass, TH1D *muongamma_invmass_mother, TH1D *eta_invmass_mother, TH1D *muon_invmass_mother, TH1D *muon_pt_mother, TH1D *muon_ptcut_mother, TH1D *gamma_pt_mother, TH1D *mu_number_event, TH1D *gamma_number_event, TH1D *total_number_event){
...
int main() {
	...
	TFile  *file_in = new TFile("gs_project33.root", "READ");

	// Open the output TFile 
	TFile  *file_out = new TFile("gs_project53.root", "recreate");
	...
	TH1D *gamma_number_event = new TH1D("gamma_number_event","#gamma number per event", 10, 0.0, 10.0);
    	gamma_number_event -> GetXaxis()-> SetTitle("event index");
	gamma_number_event -> GetYaxis()-> SetTitle("number of #gamma");

	TH1D *total_number_event = new TH1D("total_number_event","Total number of particles per event", 10, 0.0, 10.0);
...
std::cout<<"Total number of entries: "<<nentries<<std::endl;
	...
	c1->Modified();
	c1->Update();
	c1->Print("project53_etainvmass.pdf","pdf");

	muongamma_invmass_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->SaveAs("project53_muongamma_invmass_mother.pdf","pdf");

	muon_invmass_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project53_muoninvmass_mother.pdf","pdf");

	eta_invmass_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project53_etainvmass_mother.pdf","pdf");

	muon_pt_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project53_mu_pt_mother.pdf","pdf");

	muon_ptcut_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project53_mu_ptcut_mother.pdf","pdf");

	gamma_pt_mother->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project53_gamma_pt_mother.pdf","pdf");

	mu_number_event->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project53_mu_number_event.pdf","pdf");

	gamma_number_event->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project53_gamma_number_event.pdf","pdf");

	total_number_event->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project53_total_number_event.pdf","pdf");
