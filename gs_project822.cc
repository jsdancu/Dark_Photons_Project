//17/02/2018
//Program reading in the three trees generated in gs_project34.cc and gs_project64.cc and exploring the efficiency/feasibility of cuts on photon pt and p
//Program inspired from gs_project82.cc
...
void analyze_event1(FILE *file_out1, vect *v, vect1 *vmu, vect2 *vgamma, int column, int row, double mu_ptcut, double mu_pcut, double eta_cut1, double eta_cut2, double gamma_ptcut0, double gamma_pcut0, double gamma_ptcut_step, double gamma_pcut_step, double massA, double dmassA){
							...
							for(Long64_t k=0;k<nentries;k++){
								...
								if(v->id[k] == 22){
									...
									if((v->mother1[i] == v->mother1[j]) && (v->mother1[i] == v->mother1[k]) && (v->mother2[i] == v->mother2[j]) && (v->mother2[i] == v->mother2[k]) && (v->motherid1[i] == 221)){									
										...
										if((pt1>mu_ptcut) && (p1>mu_pcut) && (eta1>eta_cut1) && (eta1<eta_cut2) && (pt2>mu_ptcut) &&  (p2>mu_pcut) && (eta2>eta_cut1) && (eta2<eta_cut2) && (inv_mass < m_eta+dm_eta) && (inv_mass> m_eta-dm_eta) && (mu_invmass < massA+dmassA) && (mu_invmass> massA-dmassA)){
	...
	vmu->B[0] = vmu->B[0] + N->B[0];
	vmu->SMB[0] = vmu->SMB[0] + N->SMB[0];
...
void analyze_event_pi(FILE *file_out1, vect *v, vect1 *vmu, vect2 *vgamma, int column, int row, double mu_ptcut, double mu_pcut, double eta_cut1, double eta_cut2, double gamma_ptcut0, double gamma_pcut0, double gamma_ptcut_step, double gamma_pcut_step, double massA, double dmassA){
							...
							for(Long64_t k=0;k<nentries;k++){
								...
								if(v->id[k] == 22){
									...
									if((v->motherid1[i] != 310) && (v->motherid1[j] != 310)){
										...
										if((inv_mass < m_eta+dm_eta) && (inv_mass > m_eta-dm_eta) && (pion_invmass < massA+dmassA) && (pion_invmass> massA-dmassA)){
	...
	vmu->misID[0] = vmu->misID[0] + N->misID[0];
...
*/
	...
	TFile  *file_in1 = new TFile("gs_project34.root", "READ");
	TFile  *file_in2 = new TFile("/disk/moose/general/user72/gs_project62.root", "READ");

	// Open the output TFile 
	TFile  *file_out = new TFile("gs_project822.root", "recreate");

	FILE  *file_out1 = fopen(" results_out822.txt", "w");
	fprintf(file_out1, "Table showing signal significance of signal muons passing various cuts: \n");
	fprintf(file_out1, "gamma(pt cut)  gamma(p cut)  S(mu cuts(pt+p+eta)+mass)  S(gamma cuts(pt+p+eta))   B(mu cuts(pt+p+eta)+mass)  B(gamma cuts(pt+p+eta))   misID(mu cuts(pt+p+eta)+mass)  misID(gamma cuts(pt+p+eta))   S/sqrt(B+misID)(mu cuts(pt+p+eta)+mass)  S/sqrt(B+misID)(gamma cuts(pt+p+eta))  \n");
	...
	//TTree *T6 = (TTree*)file_in2->Get("T3");
...
std::cout<<"Total number of entries T3: "<<nentries<<std::endl;
   	for (Long64_t i=0;i<nentries;i++){
		...
		if(i+1==nentries){
			...
			analyze_event_pi(file_out1, v, vmu, vgamma, column, row, mu_ptcut, mu_pcut, eta_cut1, eta_cut2, gamma_ptcut0, gamma_pcut0, gamma_ptcut_step, gamma_pcut_step, massA, dmassA);
	...
	for(int i=0;i<column;i++){ 
		...
		for(int j=0;j<row;j++){ 
			...
			SMB = (double)vgamma->SMB[i][j] * Br;
			B = (double)vgamma->B[i][j];
			misID = (double)vgamma->misID[i][j] * RmisID;
	...
	c1->Modified();
	c1->Update();
	c1->Print("project822_ptvsp.pdf","pdf");	

	std::cout<<"Signal entries (mu cuts + mass cut): "<<vmu->S[0]<<std::endl;
	std::cout<<"SM Background entries (mu cuts + mass cut): "<<vmu->SMB[0]<<std::endl;
	std::cout<<"Background entries (mu cuts + mass cut): "<<vmu->B[0]<<std::endl;
	std::cout<<"MisID pion entries (mu cuts + mass cut): "<<vmu->misID[0]<<std::endl;

	fprintf(file_out1, "Signal entries (mu cuts + mass cut): %d \n", vmu->S[0]);
	fprintf(file_out1, "SM Background entries (mu cuts + mass cut): %d \n", vmu->SMB[0]);
	fprintf(file_out1, "Background entries (mu cuts + mass cut): %d \n", vmu->B[0]);
	fprintf(file_out1, "MisID pion entries (mu cuts + mass cut): %d \n", vmu->misID[0]);
