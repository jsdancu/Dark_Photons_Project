...
*/
	...
	TFile  *file_in2 = new TFile("/disk/moose/general/user72/gs_project65.root", "READ");
	TFile  *file_in3 = new TFile("/disk/moose/general/user72/gs_project36.root", "READ");

	// Open the output TFile 
	TFile  *file_out = new TFile("gs_project85.root", "recreate");
	...
	FILE  *file_out1 = fopen("results_out85.txt", "w");
...
std::cout<<"Total number of entries T1: "<<nentries<<std::endl;
	...
	c1->Modified();
	c1->Update();
	c1->Print("project85_ptvsp.pdf","pdf");

	S_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project85_S_ptvsp.pdf","pdf");

	B_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project85_B_ptvsp.pdf","pdf");	

	KS_number_event->Draw();

	c1->Modified();
	c1->Update();
	c1->Print("project85_KS_number_event.pdf","pdf");
