...
//std::cout<<misid1<< "     "<<misid2<<std::endl;
...
*/
	...
	TFile  *file_in2 = new TFile("/disk/moose/general/user72/friday0203/gs_project67.root", "READ");

	// Open the output TFile 
	TFile  *file_out = new TFile("gs_project87.root", "recreate");

	FILE  *file_out1 = fopen("results_out87.txt", "w");
	fprintf(file_out1, "Table showing signal significance of signal muons passing various cuts: \n");
	fprintf(file_out1, "gamma(pt cut)  gamma(p cut)   S(gamma cuts(pt+p+eta))   B_EM(gamma cuts(pt+p+eta))   B(gamma cuts(pt+p+eta))    misID(gamma cuts(pt+p+eta))    S/sqrt(B+misID)(gamma cuts(pt+p+eta))    S/sqrt(misID)(gamma cuts(pt+p+eta)) \n");
...
std::cout<<"Total number of entries T3: "<<nentries<<std::endl;
	...
	for(int i=0;i<column;i++){ 
		...
		for(int j=0;j<row;j++){ 
			...
			//SBem_ptvsp->SetBinContent(i+1, j+1, S/SMB);
			//S_ptvsp->SetBinContent(i+1, j+1, (double)vgamma->S[i][j]/(double)vmu->S[0]);
	...
	c1->Modified();
	c1->Update();
	c1->Print("project87_ptvsp.pdf","pdf");

	SBem_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project87_SBem_ptvsp.pdf","pdf");

	S_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project87_S_ptvsp.pdf","pdf");

	Bem_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project87_Bem_ptvsp.pdf","pdf");

	B_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project87_B_ptvsp.pdf","pdf");

	misID_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project87_misID_ptvsp.pdf","pdf");	
	...
	fprintf(file_out1, "Candidate type    Total entries    After mu + A' mass cut    After eta mass cut	Signal significance after mu + A' cut: \n");
