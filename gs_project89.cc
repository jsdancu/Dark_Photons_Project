//05/03/2018
//Program reading in the three trees generated in gs_project34.cc and gs_project69.cc and exploring the efficiency/feasibility of cuts on photon pt and p
//Program inspired from gs_project87.cc 
...
struct vect1{
	...
	std::vector<Long64_t> S, SMB, B, misID;
	
};

struct vect2{
	
	int S[80][100]; int SMB[80][100]; int B[80][100]; double misID[80][100];
...
void analyze_event1(FILE *file_out1, vect *v, vect1 *vmu, vect2 *vgamma, int column, int row, double mu_ptcut, double mu_pcut, double eta_cut1, double eta_cut2, double gamma_ptcut0, double gamma_pcut0, double gamma_ptcut_step, double gamma_pcut_step, double massA, double dmassA){
	...
	double inv_mass, mu_invmass, pt1, pt2, pt3, p1, p2, p3, eta1, eta2, eta3, gamma_ptcut, gamma_pcut;
	...
	for(int i=0;i<column;i++){ 
		for(int j=0;j<row;j++){ 
			...
			vgamma->SMB[i][j] = vgamma->SMB[i][j] + M->SMB[i][j];
...
void analyze_event2(FILE *file_out1, vect *v, vect1 *vmu, vect2 *vgamma, int column, int row, double mu_ptcut, double mu_pcut, double eta_cut1, double eta_cut2, double gamma_ptcut0, double gamma_pcut0, double gamma_ptcut_step, double gamma_pcut_step, double massA, double dmassA){
	...
	for(int i=0;i<column;i++){ 
		for(int j=0;j<row;j++){ 
			...
			vgamma->S[i][j] = vgamma->S[i][j] + M->S[i][j];
			vgamma->B[i][j] = vgamma->B[i][j] + M->B[i][j];
		}
	}
...
void analyze_event_pi(FILE *file_out1, vect *v, vect1 *vmu, vect2 *vgamma, int column, int row, double mu_ptcut, double mu_pcut, double eta_cut1, double eta_cut2, double gamma_ptcut0, double gamma_pcut0, double gamma_ptcut_step, double gamma_pcut_step, double massA, double dmassA, double alpha, double beta, double gamma){

	double inv_mass, pion_invmass, pt_pp, pt_pn, p_pp, p_pn, eta_pp, eta_pn, pt3, p3, eta3, gamma_ptcut, gamma_pcut;
...
*/
	...
	TFile  *file_in1 = new TFile("/disk/moose/general/user72/gs_project36.root", "READ");
	TFile  *file_in2 = new TFile("/disk/moose/general/user72/friday0203/gs_project69.root", "READ");

	// Open the output TFile 
	TFile  *file_out = new TFile("gs_project89.root", "recreate");

	FILE  *file_out1 = fopen("results_out89.txt", "w");
	fprintf(file_out1, "Table showing signal significance of signal muons passing various cuts: \n");
	fprintf(file_out1, "gamma(pt cut)  gamma(p cut)   S(gamma cuts(pt+p+eta))   B_EM(gamma cuts(pt+p+eta))   B(gamma cuts(pt+p+eta))   misID(gamma cuts(pt+p+eta))   S/B_EM(gamma cuts(pt+p+eta))  S/sqrt(B+misID)(gamma cuts(pt+p+eta))  \n");
	...
	TH2D *misID_ptvsp = new TH2D("misID_ptvsp","S/#sqrt{misID} based on p_{t} vs p cuts", column, 0.0, gamma_ptcut_step*column, row, 0.0, gamma_pcut_step*row);
	misID_ptvsp -> GetXaxis()-> SetTitle("p_{t} (GeV)");
	misID_ptvsp -> GetYaxis()-> SetTitle("p (GeV)");

	double epsilon2 = 1e-7;//kinetic mixing coupling
	double massA = 0.30;//dark photon mass
...
std::cout<<"Total number of entries T3: "<<nentries<<std::endl;
	...
	for(int i=0;i<column;i++){ 
		...
		for(int j=0;j<row;j++){ 
			...
			gamma_pcut = gamma_pcut0 + j * gamma_pcut_step;
	   		...
	   		// this is the prompt case
                        // remove DP signal from B
                       S = ((double)vgamma->SMB[i][j] + (double)vgamma->B[i][j]) * Br * F;
	               B = ((double)vgamma->SMB[i][j] + (double)vgamma->B[i][j]) * Br + (double)vgamma->misID[i][j];

			sigma = S / sqrt(B);;
	
			// this is the displaced case
                        // remove DP signal from B
			// S = ((double)vgamma->SMB[i][j] + (double)vgamma->B[i][j]) * Br * F * S[i][j] with lifetime cut /  S[i][j] without lifetime cut;
			// B = KS[i][j];

			//S = (double)vgamma->S[i][j] * F * Br * (double)vmu->SMB[0] /(double)vmu->S[0];
			//S = F * Br * (double)vmu->SMB[0] /(double)vmu->S[0];
			EMB = (double)vgamma->SMB[i][j] * Br;
			misID = (double)vgamma->misID[i][j];

			ptvsp->SetBinContent(i+1, j+1, sigma);
			misID_ptvsp -> SetBinContent(i+1, j+1, S/sqrt(misID));
			Bem_ptvsp->SetBinContent(i+1, j+1, (double)vgamma->SMB[i][j]/(double)vmu->SMB[0]);
			B_ptvsp->SetBinContent(i+1, j+1, (double)vgamma->B[i][j]/(double)vmu->B[0]);

			fprintf(file_out1, "%.5e   %.5e   %.5e   %.5e   %.5e   %.5e   %.5e   %.5e  \n", gamma_ptcut, gamma_pcut, S, EMB, B, misID, sigma, S/sqrt(misID));
	...
	c1->Modified();
	c1->Update();
	c1->Print("project89_ptvsp.pdf","pdf");

	SBem_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project89_SBem_ptvsp.pdf","pdf");

	S_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project89_S_ptvsp.pdf","pdf");

	Bem_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project89_Bem_ptvsp.pdf","pdf");

	B_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project89_B_ptvsp.pdf","pdf");

	misID_ptvsp->Draw("COLZ");

	c1->Modified();
	c1->Update();
	c1->Print("project89_misID_ptvsp.pdf","pdf");		
	...
	fprintf(file_out1, "Candidate type    Total entries    After mu + A' mass cut    After eta mass cut	Signal signaificance after mu + A' cut: \n");
	fprintf(file_out1, "S    			   %d    		%d   			%d 			%e \n", vmu->S[0], vmu->S[1], vmu->S[2], (double)vmu->S[1]/(double)vmu->S[0]);
	fprintf(file_out1, "SMB    		   %d    		%d    			%d 			%e \n", vmu->SMB[0], vmu->SMB[1], vmu->SMB[2], (double)vmu->SMB[1]/(double)vmu->SMB[0]);
	fprintf(file_out1, "B    			   %d    		%d    			%d 			%e \n", vmu->B[0], vmu->B[1], vmu->B[2], (double)vmu->B[1]/(double)vmu->B[0]);
	fprintf(file_out1, "misID    		   %d    		%d    			%d 			%e \n", vmu->misID[0], vmu->misID[1], vmu->misID[2], (double)vmu->misID[1]/(double)vmu->misID[0]);

	S = ((double)vmu->SMB[1] + (double)vmu->B[1]) * Br * F;
	B = ((double)vmu->SMB[1] + (double)vmu->B[1]) * Br + (double)vmu->misID[1];
	fprintf(file_out1, "Figure of merit after mu + A' cut (S/sqrt(B)):  %e \n", S/sqrt(B));
	fprintf(file_out1, "Figure of merit after mu + A' cut (S/sqrt(misID)):  %e \n", S/sqrt((double)vmu->misID[1]));

	/*S = (double)vmu->S[1] * F * Br * (double)vmu->SMB[0] /(double)vmu->S[0];
	SMB = (double)vmu->SMB[1] * Br;
	B = (double)vmu->B[1] * Br;
	misID = (double)vmu->misID[1];*/
	//fprintf(file_out1, "Figure of merit after mu + A' cut:  %e \n", metric(S, SMB, B, misID));
